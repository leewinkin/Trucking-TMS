...
try {
  ...
  assert.equal(customerDocuments.body.documents.some((document) => Object.hasOwn(document, "documentHostDiagnostic")), false);
  ...
  const adminShipmentDocuments = await admin.request("/api/shipments/ship_a/documents");
  assert.equal(adminShipmentDocuments.status, 200, "admin can list shipment documents");
  assert.equal(adminShipmentDocuments.body.documents.some((document) => Object.hasOwn(document, "providerReference")), true);
  assert.equal(adminShipmentDocuments.body.documents.some((document) => Object.hasOwn(document, "rawMetadata")), true);
  assert.equal(adminShipmentDocuments.body.documents.some((document) => Object.hasOwn(document, "documentHostDiagnostic")), true);
  const operationsShipmentDocuments = await operations.request("/api/shipments/ship_b/documents");
  assert.equal(operationsShipmentDocuments.status, 200, "sub-admin/operations can list any shipment documents");
  const staffShipmentDocuments = await staff.request("/api/shipments/ship_a/documents");
  assert.equal(staffShipmentDocuments.status, 403, "staff must not list shipment documents");
  assert.equal(JSON.stringify(staffShipmentDocuments.body).includes("providerReference"), false);
  assert.equal(JSON.stringify(staffShipmentDocuments.body).includes("rawMetadata"), false);
  assert.equal(JSON.stringify(staffShipmentDocuments.body).includes("documentHostDiagnostic"), false);
  assert.equal(JSON.stringify(staffShipmentDocuments.body).includes("doc_invoice_a"), false);
